...
foreach $sd_subfolder (sort { -d $a <=> -d $b } readdir($sd_path_source_handler))
{
	# Skip "." and "..".
...
if($multi_disc_game_count > 0)
{
	...
	foreach $sd_subfolder (sort { -d $a <=> -d $b } readdir($sd_path_source_handler))
	{
		...
		else
		{
			...
			foreach $sd_game_file (sort { -d $a <=> -d $b } readdir($sd_game_folder_handler))
			{
				# Skip "." and "..".
			...
			# Flatted GDI table of contents.
			$gdi_data = join("", @gdi_toc);
	...
	foreach $multi_disc_game_name (keys %multi_disc_games)
	{
		...
		for(1 ... $multi_disc_games{$multi_disc_game_name})
		{
			...
			# Iterate through game folder in alphanumeric order.
			foreach $sd_game_file (sort { -d $a <=> -d $b } readdir($sd_multi_disc_handler))
			{
				# Skip "." and "..".
				next if($sd_game_file =~ /^\.\.?/);
...
print "SD card conversion process completed!\n";
